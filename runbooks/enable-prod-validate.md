...
- Atlas cluster `vozni-${kenv}-${sbenv}` exists (prod/staging) or shared cluster
  `vozni-${kenv}` is available
- Atlas database user `${sbenv}` exists with a Terraform-managed random password

**MongoDB credential setup required (manual step, not done by Terraform):**
The Atlas database user exists, but the MongoDB connection URI has **not** been
written to Key Vault. Services in the new sub-environment cannot connect to
MongoDB until this is done. Complete this step before or immediately after Phase 6
(merge order) — pods will CrashLoopBackOff or produce auth errors until it is set.
See [infrastructure.md — MongoDB Atlas Credential Management](../infrastructure.md#mongodb-atlas-credential-management)
for the full procedure, including how to read the Terraform-managed password from
HCP Terraform state and which Key Vault secret name to use.
...
| Azure Key Vault (`vozni-${kenv}-${sbenv}`) | MongoDB connection URI secret | **Manual** — write after Terraform apply |
